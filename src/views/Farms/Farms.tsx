...
import { Image, Heading } from '@coffeeswap/uikit'
import { BLOCKS_PER_YEAR, COFE_PER_BLOCK, COFE_POOL_PID } from 'config'
...
const Farms: React.FC = () => {
  ...
  const farmsList = useCallback(
    (farmsToDisplay, removed: boolean) => {
      const cakePriceVsBNB = new BigNumber(farmsLP.find((farm) => farm.pid === COFE_POOL_PID)?.tokenPriceVsQuote || 0)
      const farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        if (!farm.tokenAmount || !farm.lpTotalInQuoteToken || !farm.lpTotalInQuoteToken) {
          return farm
        }
        const cakeRewardPerBlock = COFE_PER_BLOCK.times(farm.poolWeight)
  ...
  return (
    <Page>
      <Heading as="h1" size="lg"mb="50px" style={{ textAlign: 'center' }}>
        {TranslateString(696, 'Stake LP tokens to earn COFE')}
      </Heading>
